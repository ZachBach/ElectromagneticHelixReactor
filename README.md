...
**2. Read the paper** — [`paper/ehr-paper.pdf`](paper/ehr-paper.pdf) is committed, so no LaTeX toolchain is needed. It defines the architecture, the operating regime, and the four predictions everything else tests. (To rebuild it, run [tectonic](https://tectonic-typesetting.github.io) on `paper/ehr-paper.tex`.)
